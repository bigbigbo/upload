...
import { ID_CARD_FRONT, ID_CARD_BACK } from '../../constants/fileType';
...
class IDCardUpload extends React.Component {
  ...
  startUpload = (cardEnd, file) => {
    ...
    const targetIndex = cardEnd === ID_CARD_FRONT ? 0 : 1;
    ...
    return (
      <>
        <IDCardItem
          ...
          onUpload={([file]) => this.handleUpload(ID_CARD_FRONT, file)}
          onSelect={() => this.handleSelect(ID_CARD_FRONT)}
        ...
        <IDCardItem
          ...
          file={backend}
          uploadOptions={{ multiple: false, accept: 'image/*' }}
          onUpload={([file]) => this.handleUpload(ID_CARD_BACK, file)}
          onSelect={() => this.handleSelect(ID_CARD_BACK)}
